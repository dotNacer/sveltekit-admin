...
Without `sort`, rows follow the model's configured `defaultSort` when it has one, and primary-key descending otherwise — a default the widget supplies itself, since `findMany` has none of its own (Prisma passes `orderBy` straight through, and Drizzle emits no `ORDER BY` at all when it is empty). The read composes `scope` and `listWhere` exactly like every other dashboard widget, so a tenant-scoped model never leaks another tenant's rows into the panel.
