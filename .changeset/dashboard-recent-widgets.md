...
Without `sort`, rows come back primary-key descending — the same default `findMany` already used elsewhere. The read composes `scope` and `listWhere` exactly like every other dashboard widget, so a tenant-scoped model never leaks another tenant's rows into the panel.
