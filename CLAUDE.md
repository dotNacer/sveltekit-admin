...
- **Two distinct scoping hooks — do not conflate them.** `models[].scope` is the tenant hook: composed with `AND` into *every* read and write (list, search, detail, edit, delete, dashboard counts, relation options, plugin reads), and its equality fields are additionally imposed on create. `listWhere` (and a relation's `relations[field].where`) still scope only the **list view** (search, sidebar filters, FK filter, pagination); they are not a substitute for `scope` and never protected detail/edit/delete.
- `scope` only covers requests the handler serves, i.e. everything under `basePath`. The rest of the consuming SvelteKit app queries Prisma directly and is not scoped by anything here — don't document or imply otherwise.
- The tenant columns from `modelScopeValues` are imposed **after** the FK/m2m revalidation loops in `handleMutation`, never before. Those loops rewrite `data[scalarName]` from the submitted form, and the tenant column is usually a relation scalar (`organizationId`) — imposing first meant the form silently won, which was a live cross-tenant write. In that final loop the assignment is unconditional; the comparison only decides whether to throw. Folding it into an `else` reintroduces the hole. Only a value the client actually asserted is compared: an empty field is what a create form renders, not a claim to another tenant.
- The tenant root model (`Organization`…) belongs in `exclude`. FK revalidation checks a submitted target against *that target model's* scope, so leaving the tenant root visible and unscoped makes every existing tenant id an acceptable value.
...
- Plugin page context has no `adapter`. Record payloads from `loadRecord` / `listRecords` / the preloaded `record` are redacted with `redactForAudit` (`hidden` + `isSensitiveFieldName`). Out-of-scope `:id` is 404 before `render`.
