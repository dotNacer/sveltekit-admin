...
import {
  ...
  isFlatEqMap,
  normalizeScope,
} from "../../../src/lib/server/adapters/filter.js";
...
describe("isLeafFilter / isCompositeFilter — discrimination structurelle", () => {
  it("leaf reconnue seulement avec op whitelisté + field string", () => {
    ...
    expect(
      isLeafFilter({ op: "containsExact", field: "name", value: "x" }),
    ).toBe(true);
...
describe("isFlatEqMap / normalizeScope", () => {
  it("objet plat de scalaires → eq (une clé) ou and (plusieurs)", () => {
    ...
    expect(normalizeScope({ tenantId: 1 })).toEqual({
      op: "eq",
      field: "tenantId",
      value: 1,
    });
  ...
  it("accepte Date, bigint, null comme scalaires ; refuse undefined, objet, tableau, {}", () => {
    expect(isFlatEqMap({ at: new Date("2020-01-01T00:00:00.000Z") })).toBe(
      true,
    );
    expect(isFlatEqMap({ n: 1n })).toBe(true);
    expect(isFlatEqMap({ x: null })).toBe(true);
    expect(isFlatEqMap(null)).toBe(false);
    expect(isFlatEqMap([])).toBe(false);
    expect(isFlatEqMap({ op: "eq", field: "id", value: 1 })).toBe(false);
    expect(isFlatEqMap({ op: "and", clauses: [] })).toBe(false);
